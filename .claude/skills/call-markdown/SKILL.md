...
1. **Deterministic pre-pass — BEFORE any agents (plain code, no LLM).** Three
   ...
   - **One subtitle per sentence.** Two sub-steps that together give
     ...
     1. **Split** each cue at every internal sentence terminal — `.` `?` `!`
        **or `…`** (optionally a closing quote) followed by whitespace. A cue can
        ...
        edges).
     2. **Stitch** a sentence the ASR split mid-word across cues: merge a piece
        into the running block **only while that block does not yet end at a
        terminal** (`[.?!…]`). As soon as it ends at a terminal — or the speaker
        changes — close it.
     **Do not** apply a "~N sentences per block" cap and **do not** merge through
     `…` (a trailing-off is a real break). The validator's `TERMINAL` is `[.?!…]`,
     so a `…`-ended subtitle is complete (no warn); only one ending **mid-word**
     before a same-speaker block warns — a true split to stitch.
